...
# Plain Muon on LLaMA-350M. Hyperparameters mirror scripts/350M/Muon.sh; the
# only intentional difference is --save_every (see the launch block below).
#
...
# 128 x 4 ranks = 512 = TOTAL_BATCH, so grad_accum is 1, matching Muon.sh.
# ~41GB of the 46GB card; drop to MICRO_BATCH=64 (grad_accum 2, identical
# optimization) if it OOMs.
MICRO_BATCH=${MICRO_BATCH:-64}
TOTAL_BATCH=${TOTAL_BATCH:-512}       # global batch, held fixed across GPU counts
LRMUON=${LRMUON:-5e-3}             # matches the stock 350M Muon.sh
# Schedule defaults come from the stock 350M Muon.sh. The 1B campaign and
# scripts/350M/Soren_slurm.sh run 15000/1500/150 instead -- for a like-for-like
# comparison against those, use:
#   STEPS=15000 WARMUP=1500 EVAL_EVERY=150 sbatch --export=ALL,STEPS,WARMUP,EVAL_EVERY ...
STEPS=${STEPS:-10000}
WARMUP=${WARMUP:-1000}
EVAL_EVERY=${EVAL_EVERY:-1000}
RUN_NAME=${RUN_NAME:-muon_350m_muonlr_${LRMUON}_s${STEPS}}
